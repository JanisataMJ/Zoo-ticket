...
const Update = () => {
  ...
  const onFinish = async (values: any) => {
    ...
    values.ID = vehicleData.ID; // Pass the vehicle ID
    const file = fileList.length > 0 ? fileList[0].originFileObj : null; // Get the file from fileList
    ...
    try {
      const res = await UpdateVehicle(values, file); // Update vehicle API
      if (res) {
        messageApi.open({
          type: 'success',
          content: res.message,
